...
def simulate_from_start(start_star, restart_star, protect_start=None, buy_back_below_start=False, buy_back_17_to_16=False, target=20, simulations=100000):
    ...
    cost_variance = sum((x - avg_cost) ** 2 for x in total_costs) / simulations
    cost_std_dev = math.sqrt(cost_variance)
    destroys_variance = sum((x - avg_destroys) ** 2 for x in total_destroys) / simulations
    destroys_std_dev = math.sqrt(destroys_variance)
    return avg_cost, avg_destroys, cost_std_dev, destroys_std_dev
...
for start_star in [15, 16, 17, 18, 19]:
    for protect_start in protect_options:
        for buy_back_below_start in drop_options:
            ...
            avg_cost, avg_destroys, cost_std_dev, destroys_std_dev = simulate_from_start(start_star, start_star, protect_start, buy_back_below_start, False, target, simulations)
            ...
            results[strategy_key] = (avg_cost, avg_destroys, cost_std_dev, destroys_std_dev)
            print(f"平均成本 = {avg_cost:.2f} 楓點 (標準差: {cost_std_dev:.2f})")
            print(f"平均破壞次數 = {avg_destroys:.2f} 次 (標準差: {destroys_std_dev:.2f})")
...
for start_star in [17, 18, 19]:
    for protect_start in [16, 17, 18, 19]:
        for buy_back_below_start in drop_options:
            ...
            avg_cost, avg_destroys, cost_std_dev, destroys_std_dev = simulate_from_start(start_star, start_star, protect_start, buy_back_below_start, True, target, simulations)
...
cost_std_dev = 0
destroys_std_dev = 0
results["20_protect_none_drop_continue_buyback1716_false"] = (avg_cost, avg_destroys, cost_std_dev, destroys_std_dev)
...
# 整理所有模擬情境
print("\n=== 所有模擬情境（共85種） ===")
for strategy_key, (avg_cost, avg_destroys, cost_std_dev, destroys_std_dev) in sorted(results.items(), key=lambda x: x[1][0]):
    ...
    print(f"{desc}：平均成本 = {avg_cost:.2f} 楓點 (標準差: {cost_std_dev:.2f})，平均破壞次數 = {avg_destroys:.2f} 次 (標準差: {destroys_std_dev:.2f})")

# 最佳策略
print("\n=== 策略比較 ===")
best_strategy = min(results, key=lambda x: results[x][0])
best_cost, best_destroys, best_cost_std_dev, best_destroys_std_dev = results[best_strategy]
...
print(f"平均成本 = {best_cost:.2f} 楓點 (標準差: {best_cost_std_dev:.2f})")
print(f"平均破壞次數 = {best_destroys:.2f} 次 (標準差: {best_destroys_std_dev:.2f})")
